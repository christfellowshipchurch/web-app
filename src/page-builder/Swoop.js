    ...
    case 'WebsiteFeature':
      return (
        <div className={classnames('col', 'px-4')}>
          <Feature name={get(item, 'feature', '')} background={bg} {...item} isRsvp />
        </div>
      );
    default:
...
const Swoop = ({ title, backgroundColors, backgroundImages, swoopTypes }) => {
  ...
  const blockItems = mapEdgesToNodes(
    get(data, 'getWebsitePageContentByTitle.childContentItemsConnection', [])
  );
  ...
  return blockItems.map((item, i) => {
    ...
    const ignoreSwoop =
      camelCase(get(item, 'contentLayout', '')).includes('background') ||
