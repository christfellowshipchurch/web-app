...
const TabIcon = ({ icon, title, onClick, isActive, eventKey, className }) => (
...
const Tabs = ({ children, className }) => {
  ...
  return (
    <Accordion className={classnames('container-fluid', className)} defaultActiveKey="0">
      <div className={classnames('row', 'justify-content-center', 'overflow-x-auto')}>
        <div className={classnames('col-12', 'col-md-6', 'overflow-x-hidden')}>
          <VisibilitySensor
            onChange={(isVisible) => {
      ...
      {children.length ? (
        children.map((n, i) => (
          <Accordion.Collapse eventKey={i.toString()} key={i} className="bg-white">
            <div className="row">
              <div className="col-12">{n}</div>
            </div>
          </Accordion.Collapse>
        ))
      ) : (
