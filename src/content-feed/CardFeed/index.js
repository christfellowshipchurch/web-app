...
import { ContentContainer, Loader } from '../../ui';
import ContentCardConnected from '../../content-card-connected';
import { GET_CONTENT_FEED } from '../queries';

const CardFeed = ({ id, title, connection, urlBase, first, className }) => {
  ...
  const calculatedFirst = first ? first + 1 : null;
  ...
  let content = get(data, `node.${connection}ContentItemsConnection.edges`, [])
    .map((edge) => edge.node)
    .filter((n) => n.id !== id);
  ...
  return content.length > 0 ? (
    <div className={classnames('container-fluid', 'max-width-1100', 'my-6', className)}>
      <div className="row">
        <div className="col">
          <h3 className="text-dark align-self-start">{title}</h3>
        </div>
        {!!urlBase && urlBase !== '' && (
          <div className="col text-right">
            <a href={`/${urlBase}`} className="text-dark align-self-end">
              See More
            </a>
          </div>
        )}
...
CardFeed.propTypes = {
  id: PropTypes.string,
  connection: PropTypes.oneOf(['child', 'sibling']),
