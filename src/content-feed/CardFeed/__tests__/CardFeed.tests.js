...
describe('CardFeed', () => {
  ...
  it('renders a sibling feed from a content item', async () => {
    act(() => {
      component = render(
        <MockedProvider mocks={[SIBLING_ITEM_FEED_MOCK]} addTypename={false}>
  ...
  it('renders a child feed from a content item', async () => {
    act(() => {
      component = render(
        <MockedProvider mocks={[CHILD_ITEM_FEED_MOCK]} addTypename={false}>
  ...
  it('renders a child feed from a content channel', async () => {
    act(() => {
      component = render(
        <MockedProvider mocks={[CHILD_CHANNEL_FEED_MOCK]} addTypename={false}>
  ...
  it('receives 0 results back from the server', async () => {
    act(() => {
      component = render(
        <MockedProvider mocks={[EMPTY_RESULTS_MOCK]} addTypename={false}>
          <CardFeed id={CHANNEL_ID} title="Channel Feed" />
        </MockedProvider>
      );
    });
  ...
  it('renders the error state', async () => {
    act(() => {
      component = render(
        <MockedProvider mocks={[FEED_ERROR]}>
          <CardFeed id={ITEM_ID} />
        </MockedProvider>
      );
    });
