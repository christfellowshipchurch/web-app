...
describe('ArticleDetail', () => {
  it('renders without crashing', () => {
    act(() => {
      render(
        <MockedProvider
          mocks={[ARTICLE_DETAIL_MOCK, ARTICLE_CATEGORIES_MOCK]}
          addTypename={false}
        >
  ...
  it('renders the error state', async () => {
    act(() => {
      component = render(
        <MockedProvider mocks={[ARTICLE_DETAIL_ERROR, ARTICLE_CATEGORIES_ERROR]}>
          <ArticleDetail match={{ params: { articleTitle: 'article-1' } }} />
        </MockedProvider>
      );
    });
  ...
  conditionalFields.forEach((n) => {
    it(`renders an article without ${n.key}`, async () => {
      act(() => {
        ...
        component = render(
          <MockedProvider mocks={[mocks, ARTICLE_CATEGORIES_MOCK]} addTypename={false}>
            <ArticleDetail match={{ params: { articleTitle: 'article-1' } }} />
          </MockedProvider>
        );
      });
