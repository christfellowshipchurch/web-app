...
const SearchFeed = ({ content }) => {
  ...
  if (content && firstHalf)
    return (
      <div className="row">
        <div className="col-12 col-md-6">
          {firstHalf.map((n, i) => (
            <ContentCardConnected key={i} contentId={n.id} card={TileRowCard} />
          ))}
        </div>
        {secondHalf && (
          <div className="col-12 col-md-6">
...
const SearchFeedConnected = ({ searchText }) => {
  ...
  if (error || data === undefined || content.length < 1)
    return <ErrorFeed searchText={searchText} />;

  return <SearchFeed content={content} />;
};
