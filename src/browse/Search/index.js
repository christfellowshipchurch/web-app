...
const Search = ({ onChange, focus }) => {
  ...
  return (
    <div className="w-100 mb-3">
      ...
      >
        <Icon name="search" fill="#828282" className="mr-2" />
        <input
          id="searchBox"
          className={classnames('h2', 'w-100', 'mb-0')}
          placeholder={'Search'}
          value={value}
          onFocus={(e) => {
            e.preventDefault();
      ...
      {isFocused && (
        <div
          ...
          className={classnames('container-fluid', 'mb-4', 'rounded', 'h-100')}
        >
          {value && value !== '' ? (
            <SearchFeed searchText={value} />
          ) : (
            <PreSearchFeed hide={onClear} />
          )}
        </div>
      )}
    </div>
