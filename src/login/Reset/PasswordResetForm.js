...
import { LockAlt, Envelope } from '../../ui/Icons';
...
const validation = {
  ...
  confirmationCode: (value) =>
    value && value !== '' ? false : 'Enter in a confirmation code',
  password: (value) => (value && value !== '' ? false : 'Enter a new password'),
...
const classes = {
  subtitle: classnames('mt-5', 'mb-3', 'text-muted'),
  row: classnames('row', 'my-4'),
};

const ResetPassword = ({ title, description, location: { search } }) => {
  const [requestPasswordChange] = useMutation(REQUEST_PASSWORD_CHANGE);
  const parsedQueryStrings = QueryString.parse(search);
  const { values, setValue, errors, submitting, setSubmitting, setError } = useForm({
    validation,
    defaultValues: {
  ...
  const onClick = async () => {
    ...
    try {
      ...
      setError(
        'confirmationCode',
        'Make sure you entered in the right Confirmation Code'
      );
      setSubmitting(false);
    }
  };
            ...
            <Card>
              ...
              <div className="container-fluid">
                <h5 className={classes.subtitle}>
                  Enter your Email Address and the Confirmation Code that was sent to your
                  email.
                </h5>
                ...
                <div className={classes.row}>
                  <div className="col">
                    <TextInput
                      ...
                      type="number"
                      onChange={(e) =>
                        setValue('confirmationCode', get(e, 'target.value', ''))
                      }
                ...
                <div className={classes.row}>
                  <div className="col">
                    <TextInput
                      ...
                      onChange={(e) =>
                        setValue('confirmPassword', get(e, 'target.value', ''))
                      }
